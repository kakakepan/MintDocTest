...
| Success | 201  | The command is "In Process"                                    | - Register Domain/n- Transfer Domain                  | The request is left as "In Process".                                                                                                    |
| Error   | 202  | The command is "Pending"                                       | - Register Domain/n- Transfer Domain                  | The command has been successfully processed for the request, but the status is "Pending".                                               |
| Error   | 214  | Contact/Host name not available.                               | - Check Contact/n- Check Host                        | The entered name is not available to be used for Contact or Host.                                                                        |
| Success | 215  | Contact/Host name available.                                   | - Check Contact/n- Check Host                        | The entered name is available to be used for Contact or Host.                                                                            |
...
| Error   | 506  | Contact/Host already in use by a domain.                        | - Modify Host/n- Delete Contact                      | The contact or host you are attempting to use is already associated with a domain.                                                       |
| Error   | 515  | Blocked Domains                                                | - Register Domains/n- Transfer In Domains/n- Transfer Domain | This error displays the list of blocked domains.                                                                                        |
| Error   | 516  | Registered within 60 days                                      | - Transfer In Domains/n- Transfer Lock/n- Transfer Domain | The commands cannot be run for a domain that has transferred or registered within 60 days.                                                |
| Error   | 517  | Incorrect Format                                               | - Create Host/n- Modify Host                          | Please provide the host name in the following format: "ns1.docu-test-case.com". Ensure that the domain host is associated with an active domain managed by the reseller. |
| Error   | 518  | Duplicated host names                                          | Create Host                                             | The host name cannot be created because it is already in use.                                                                            |
| Error   | 519  | Incorrect Format                                               | - Create Host/n- Modify Host                          | Enter the IP address of the host in either IPV4 or IPV6 format. If the entered format is incorrect, an error will be displayed.          |
...
| Error   | 570  | Validation Error Message                                       | - Add Fields/n- Add Documentation/n- Renew Domain  | The entered data of the reseller does not pass validation due to an unsupported registration period.                                 |